...
test_ParseRights(void)
{
    ...
    code = aclutil_ParseRights("rl", 0, &rtype, &rights, &offset);
    ...
    /* AFS shorthands */
    code = aclutil_ParseRights("read", 0, &rtype, &rights, &offset);
    is_int(0, code, "ParseRights succeeds for \"read\"");
    is_int(PRSFS_READ | PRSFS_LOOKUP, rights,
	   "... and produces correct rights bitmask");
    is_int(add, rtype, "... with correct default rtype add");

    code = aclutil_ParseRights("mail", 0, &rtype, &rights, &offset);
    is_int(0, code, "ParseRights succeeds for \"mail\"");
    is_int(PRSFS_INSERT | PRSFS_LOCK | PRSFS_LOOKUP, rights,
	   "... and produces correct rights bitmask");
    is_int(add, rtype, "... with correct default rtype add");

    code = aclutil_ParseRights("write", 0, &rtype, &rights, &offset);
    ...
    code = aclutil_ParseRights("all", 0, &rtype, &rights, &offset);
    ...
    code = aclutil_ParseRights("none", 0, &rtype, &rights, &offset);
    ...
    code = aclutil_ParseRights("null", 1, &rtype, &rights, &offset);
    ...
    /* Error handling */
    offset = -1;
    code = aclutil_ParseRights("bogus", 0, NULL, &rights, &offset);
    is_int(EINVAL, code,
	   "ParseRights fails for NULL rtype pointer with EINVAL");

    code = aclutil_ParseRights("bogus", 0, &rtype, NULL, &offset);
    is_int(EINVAL, code,
	   "ParseRights fails for NULL output rights pointer with EINVAL");

    code = aclutil_ParseRights(NULL, 0, &rtype, &rights, &offset);
    is_int(EINVAL, code,
	   "ParseRights fails for NULL rights pointer with EINVAL");

    code = aclutil_ParseRights("rlbogus", 0, &rtype, &rights, &offset);
...
test_StringifyRights(void)
{
    ...
    str = aclutil_StringifyRights(PRSFS_READ | PRSFS_LOOKUP, 0, &buf);
    is_string("rl", str, "StringifyRights converts READ|LOOKUP to \"rl\"");

    str = aclutil_StringifyRights(PRSFS_READ | PRSFS_WRITE | PRSFS_ADMINISTER,
				  0, &buf);
    is_string("rwa", str,
	      "StringifyRights converts READ|WRITE|ADMINISTER to \"rwa\"");

    str = aclutil_StringifyRights(PRSFS_READ | PRSFS_USR0 | PRSFS_USR3,
				  0, &buf);
    is_string("rAD", str, "StringifyRights converts READ|USR0|USR3 to \"rAD\"");

    /* No rights and full rights */
    str = aclutil_StringifyRights(0, 0, &buf);
    is_string("", str, "StringifyRights converts no rights to empty string");

    str = aclutil_StringifyRights(PRSFS_READ | PRSFS_LOOKUP | PRSFS_INSERT |
    ...
    /* Error handling */
    str = aclutil_StringifyRights(PRSFS_READ, 0, NULL);
...
test_ParseAcl(void)
{
    ...
    code = aclutil_ParseAcl(astr, &acl);
...
test_EmptyAcl(void)
{
    ...
    aclutil_ZapAcl(&empty);

    /* Create with simple DCE/DFS ACL string header as input */
    code = aclutil_EmptyAcl("0 dfs:1 example.com\n2\n", &empty);
    ...
    /* Error handling */
    code = aclutil_EmptyAcl(NULL, &empty);
...
test_FindList(void)
{
    ...
    entry = aclutil_FindList(acl->pluslist, "system:administrators");
    ok(entry != NULL,
    ...
    entry = aclutil_FindList(acl->minuslist, "hacker");
    ok(entry != NULL,
    ...
    entry = aclutil_FindList(acl->pluslist, "TESTER");
    ok(entry != NULL,
    ...
    /* Nonexistent name */
    entry = aclutil_FindList(acl->pluslist, "bogus");
    ok(entry == NULL, "... and FindList returns NULL for nonexistent name");
    aclutil_ZapAcl(&acl);

    /* Error handling */
    entry = aclutil_FindList(NULL, "bogus");
...
test_AclToString(void)
{
    ...
    code = aclutil_AclToString(acl, buf, sizeof(buf));
    is_int(0, code,
...
test_ChangeList(void)
{
    ...
    code = aclutil_ChangeList(acl, 1, "tester", PRSFS_READ, NULL);
    ...
    rtype = reladd;
    code = aclutil_ChangeList(acl, 1, "tester", PRSFS_LOOKUP, &rtype);
    ...
    rtype = reldel;
    code = aclutil_ChangeList(acl, 1, "tester", PRSFS_LOOKUP | PRSFS_ADMINISTER,
			      &rtype);
    ...
    rtype = reldel;
    code = aclutil_ChangeList(acl, 1, "tester", PRSFS_READ, &rtype);
    ...
    code = aclutil_ChangeList(acl, 1, "newuser", 0, NULL);
    ...
    rtype = reldel;
    code = aclutil_ChangeList(acl, 1, "bogus", PRSFS_READ, &rtype);
    is_int(0, code,
	   "ChangeList attempts to modify nonexistent entry with reldel rtype");
    entry = aclutil_FindList(acl->pluslist, "bogus");
    ok(entry == NULL, "... and the entry is not present in the pluslist");
    entry = aclutil_FindList(acl->minuslist, "bogus");
    ...
    code = aclutil_ChangeList(acl, 0, "hacker", PRSFS_READ, NULL);
    ...
    code = aclutil_ChangeList(acl, 1, longname, PRSFS_READ, NULL);
