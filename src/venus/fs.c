...
SetACLCmd(struct cmd_syndesc *as, void *arock)
{
	...
	if (ta)
	    aclutil_ZapAcl(&ta);
	code = aclutil_ParseAcl(space, &ta);
	opr_Assert(code == 0);
	if (!plusp && ta->dfs) {
	    fprintf(stderr,
	...
	if (ta)
	    aclutil_ZapAcl(&ta);
	if (clear) {
	    code = aclutil_EmptyAcl(space, &ta);
	    opr_Assert(code == 0);
	}
	else {
	    code = aclutil_ParseAcl(space, &ta);
	...
	for (ui = as->parms[1].items; ui; ui = ui->next->next) {
	    ...
	    code = aclutil_ParseRights(ui->next->data, ta->dfs, &rtype, &rights,
				       &idx);
	    if (code != 0) {
		if (idx != -1) {
		...
		if (!aclutil_FindList(tlist, ui->data))
		    continue;
	    }
	    ...
	    if (rtype == destroy && ta->dfs)
		rights = -1;
	    code = aclutil_ChangeList(ta, plusp, ui->data, rights, &rtype);
	    opr_Assert(code == 0);
	}
	code = aclutil_AclToString(ta, buf, sizeof(buf));
	opr_Assert(code == 0);
...
CopyACLCmd(struct cmd_syndesc *as, void *arock)
{
    ...
    code = aclutil_ParseAcl(space, &fa);
    opr_Assert(code == 0);

    code = GetCell(as->parms[0].items->data, cell);
    if (code == 0) {
	aclutil_CleanAcl(fa, cell);
    }
	...
	for (tp = fa->pluslist; tp; tp = tp->next) {
	    code = aclutil_ChangeList(ta, 1, tp->name, tp->rights, NULL);
	    opr_Assert(code == 0);
	}
	for (tp = fa->minuslist; tp; tp = tp->next) {
	    code = aclutil_ChangeList(ta, 0, tp->name, tp->rights, NULL);
	    opr_Assert(code == 0);
	}
	code = aclutil_AclToString(ta, buf, sizeof(buf));
	opr_Assert(code == 0);
...
CleanACLCmd(struct cmd_syndesc *as, void *arock)
{
	...
	code = GetCell(ti->data, cell);
	if (code == 0) {
	    changes = aclutil_CleanAcl(ta, cell);
	}

	if (changes) {
	    struct acl_stringbuf buf;
	    /* now set the acl */
	    code = aclutil_AclToString(ta, acl_str, sizeof(acl_str));
	    opr_Assert(code == 0);
		...
		for (te = ta->pluslist; te; te = te->next) {
		    printf("  %s ", te->name);
		    printf("%s\n",
			   aclutil_StringifyRights(te->rights, ta->dfs, &buf));
		}
	    }
	    if (ta->nminus > 0) {
...
GetCallerAccess(struct cmd_syndesc *as, void *arock)
{
	...
	printf("%s\n", aclutil_StringifyRights(stat.callerAccess, 0, &buf));
    }
    return error;
}
