 ...
 */
char *
aclutil_StringifyRights(afs_int32 rights, int is_dfs,
			struct acl_stringbuf *a_strbuf)
{
    ...
    buf_i = 0;
    for (acl_i = 0; bitmap_order[acl_i] != '\0'; acl_i++) {
	unsigned char r = bitmap_order[acl_i];
	if (is_dfs && r == 'A') {
	...
	if ((rights & bitmap_table[r]) != 0) {
	    a_strbuf->sbuf[buf_i] = r;
	    buf_i++;
 ...
 */
int
aclutil_ParseRights(const char *rights, int is_dfs, enum rtype *a_rights_type,
 ...
 */
int
aclutil_CleanAcl(struct Acl *aa, char *cellname)
{
 ...
 */
struct AclEntry *
aclutil_FindList(struct AclEntry *alist, const char *aname)
{
    while (alist) {
 ...
 */
int
aclutil_ChangeList(struct Acl *al, afs_int32 plus, const char *aname,
		   afs_int32 arights, const enum rtype *artypep)
{
    size_t namelen;
    struct AclEntry *tentry;
    if (plus) {
	tentry = aclutil_FindList(al->pluslist, aname);
    } else {
	tentry = aclutil_FindList(al->minuslist, aname);
    }
 ...
 */
int
aclutil_ParseAcl(const char *astr, struct Acl **a_acl)
{
    size_t namelen;
    int code;
 ...
 * recovered from. See aclutil_ParseAcl for information on expected ACL string
 * format.
 *
 * The caller is responsible for freeing the newly created Acl struct by
 * invoking aclutil_ZapAcl.
 ...
 */
int
aclutil_EmptyAcl(const char *astr, struct Acl **a_acl)
{
    struct Acl *tp = NULL;
    int code, junk;
 ...
 */
int
aclutil_AclToString(const struct Acl *acl, char *a_acl_str, size_t len)
{
    char *buf = a_acl_str;
 ...
 */
void
aclutil_ZapAcl(struct Acl **a_acl)
{
    struct Acl *acl;
